...
> 此规则每轮注入，防止长上下文时遗忘 Impetus 流程状态。
> Hook 平台额外执行 `impetus-hook-guard.sh` 进行硬性拦截；
> 此 Rule 是所有平台通用的软性防线。
...
有活跃 impetus change 时（`openspec/changes/<name>/.impetus.yaml` 存在），**每次开始执行操作前**必须读取 `phase` 字段确认当前阶段。
...
- **阶段退出**: `impetus-guard <name> <phase> --apply`（必须看到 ALL CHECKS PASSED）
- **压缩恢复**: `impetus-state check <name> <phase> --recover`
- **状态更新**: 关键操作后通过 `impetus-state set` 更新字段，禁止手工编辑 .impetus.yaml
- **handoff 生成**: `impetus-handoff <name> design --write`（禁止手写摘要）
...
1. 第一个脚本操作 = `impetus-handoff <name> design --write`（未生成 handoff 禁止加载 brainstorming）
...
2. 每个 task 验收后必须更新 tasks.md 与证据。`executing-plans` 将 task 进度保留在工作区，等实现 Review 窗口通过后一次提交；不得按 task 反复请求提交。`subagent-driven-development` 必须构建依赖图，将独立 task 按 wave 并行派发；每个 task 必须等 spec compliance 与 code quality 两个审查都通过，再由协调者按任务唯一文本定向勾选和验证；task/wave 提交必须预授权或再次明确确认。不得用未完成任务总表代替当前任务验证
3. 遇到失败必须加载 **systematic-debugging** skill，根因未定位前不得提出源码修复
4. spec 变更分级: 小改直接编辑 | 中改加载 brainstorming | 大改暂停等用户确认拆分

## Verify 阶段专项

1. 第一步运行 `impetus-state scale <name>` 确定验证级别
...
```bash
"$IMPETUS_BASH" "$IMPETUS_STATE" check <name> <phase> --recover
```
...
2. 读取 `impetus/reference/subagent-dispatch.md` 获取 Impetus 专属扩展 (re-read `impetus/reference/subagent-dispatch.md` for Impetus-specific extensions)
3. 读取 `openspec/changes/<name>/.impetus/subagent-progress.md` 恢复依赖图、当前 wave、每个 task 的精确阶段、证据和审查-修复轮次 (Read `openspec/changes/<name>/.impetus/subagent-progress.md` to recover the dependency graph, current wave, exact per-task stage, evidence, and review-fix round)
4. 禁止在主会话中直接执行 task (Do not execute the pending task directly in the main window)
5. 按检查点恢复；缺失或不匹配时才重建依赖图并计算第一个可运行 wave
6. 已提交但未通过双审查的 task 保持未勾选，继续审查/修复循环
7. wave 的双审查和定向勾选验证通过后，立即计算并派发下一个可运行 wave，不得总结或询问是否继续

## 阶段退出后自动过渡

guard `--apply` 成功后，必须调用下一阶段的 skill：

- open → `impetus-design`（full）/ `impetus-build`（hotfix/tweak）
- design → `impetus-build`
- build → `impetus-verify`
- verify → `impetus-archive`
