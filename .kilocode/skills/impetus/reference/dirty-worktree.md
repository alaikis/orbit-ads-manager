...
规范路径：`impetus/reference/dirty-worktree.md`

本协议由所有涉及代码修改的 impetus 子 skill 共享。当 agent 恢复上下文或继续执行时，必须按本协议处理未提交的工作区改动。各子 skill 可在本协议基础上定义阶段特例（如 verify 阶段对实现改动的特殊处理），详见对应子 skill 文件。本文件不重复阶段特例。

## 1. 检查步骤

继续或开始修改前，必须先解析目标 git 仓库并对**每个**仓库运行检查。Impetus 可能运行在非 git 父目录下、并列多个子仓库：
...
单仓库（single）模式下 `REPOS` 只含一个仓库，等价于在当前仓库直接运行上述命令。必要时再逐仓库查看 `git -C "$repo" diff` / `git -C "$repo" diff --cached` / 新建文件内容。
...
- dirty worktree 只代表代码事实，不会自动推进 `.impetus.yaml` 的 `phase` 或勾选 `tasks.md`；只有完成归因、验证、同步必要文档，并通过对应阶段 guard 后，才允许推进 Impetus 状态
